...
export abstract class BaseModel {
  ...
  abstract presentLogic(store: { [Prop in keyof this["publicStore"]]?: extractGeneric<this["publicStore"][Prop]> }, any?:any): void
  //Procedure to mutate the store following SAM principles
  present(store: { [Prop in keyof this["publicStore"]]?: extractGeneric<this["publicStore"][Prop]> }): true | undefined {
    this.presentLogic(store)
...
export type PublicStoreValues<T extends BaseModel> = { [Prop in keyof T["publicStore"]]?: extractGeneric<T["publicStore"][Prop]> }


export function BuildClosedModel<T extends BaseModel>(model: T): ClosedModel<T> {
  return new ClosedModelImpl(model);
}
class ClosedModelImpl<T extends BaseModel> {
  private model: T;
  public present: any;
...
export interface ClosedModel<T extends BaseModel>{
  present: T["present"]
